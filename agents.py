...
- 語尾「〜ものだ」「〜ものだった」「〜のだ」「〜のだった」は使わない——AIの典型的な締め口癖
...
KAIKOWA_SYNTAX_TEMPLATES = [
    {
        "id": "series",
        "name": "日常どんでん返し",
        "desc": "ごく普通の日常描写→最後の1〜2文でどんでん返し",
        "instruction": """普通の日常の一場面から始め、最後の1〜2文でどんでん返し。
構成: ①いつ・どこで・何をした（1〜2文）→②会話や出来事（1〜2文）→③オチの事実（1〜2文）。
時系列を明確に：「〜したとき」「〜したら」「〜したあと」で前後関係をわかりやすくする。
禁止: ——（ダッシュ）演出・ハッシュタグ・感情説明・絵文字・「死んだ」（→「亡くなった」を使う）。
文体: 淡々とした事実の積み重ね。素っ気なく書く。150〜220字。""",
        "example": "深夜、台所で水を飲んだ。コップを置いたとき、隣の部屋から物音がした。\n\n「ねえ、まだ起きてたの？」\n\n妻の声だった。妻は今日から実家に帰っている。",
    },
    {
        "id": "jitsuwa",
        "name": "実話体験談",
        "desc": "一人称・具体的状況・淡々と・余韻で終わる",
        "instruction": """一人称・過去形。「いつ・どこで」を最初の1文に書く。
出来事を時系列で書く——「〜したら」「〜したとき」「〜したあと」で順番がわかるようにする。
禁止: 「怖かった」「鳥肌」などの感情説明・ハッシュタグ・絵文字・——（ダッシュ）演出・「死んだ」（→「亡くなった」）。
最後は「それだけだった。」「あれ以来、行っていない。」などの短い1文で終わる。150〜220字。""",
        "example": "去年の夏、祖母の家に泊まった。夜中に喉が渇いて廊下に出たら仏間の電気がついてた。\n\n引き戸を開けたら座布団に誰かが正座した跡があった。まだ温かかった。\n\n翌朝祖母に言ったら黙って手を合わせた。それだけだった。",
    },
    {
        "id": "imikowa",
        "name": "意味がわかると怖い",
        "desc": "表面は普通→読み返すと気づく仕掛け・問いかけで締め",
        "instruction": """全体を「普通の日常エピソード」として書く——怖い描写は一切しない。
読み返すと「あ、おかしい」と気づく仕掛けを1つだけ埋め込む（説明しない）。
最後の1文は必ず「…気づいた方はいますか？」にする。
禁止: ——（ダッシュ）演出・ハッシュタグ・絵文字・仕掛けの説明。150〜220字。""",
        "example": "夜道で財布を落として、見知らぬ女の人が拾って渡してくれた。\n\n家帰って中身確認したら全部あった。写真も全部。ただ一枚だけ知らない写真が増えてた。\n\n…気づいた方はいますか？",
    },
    {
        "id": "suuji",
        "name": "数字フック",
        "desc": "「3日前から」「午前3時に」など数字で始まる",
        "instruction": """冒頭を必ず「[数字]日前から」「午前[数字]時[数字]分」など数字で始める。
「3日前→昨日→今朝」のように日付・時間で変化を追う。各段落2〜3文。最後の1〜2文でオチ。
時系列を明確に：「昨日」「今朝」「次の日」など具体的な時間軸を使う。
禁止: ——（ダッシュ）演出・感情説明・ハッシュタグ・絵文字・「死んだ」（→「亡くなった」）。淡々と事実だけ書く。150〜220字。""",
        "example": "3日前から玄関の靴の向きが変わってる。\n\n最初は気のせいだと思ってた。昨日帰ったら自分の靴だけ外向きに揃えられてた。\n\n今朝も変わってた。",
    },
    {
        "id": "といかけ",
        "name": "問いかけ終わり",
        "desc": "読者を巻き込む・「あなたは〜しましたか？」系の締め",
        "instruction": """誰でも経験しそうな日常の場面から始める。
出来事の描写のみ——「怖い」「不思議な」などの形容詞なし。
最後は「あなたは〜しましたか？」「確かめましたか。」などの問いかけ2行で終わる。
禁止: ——（ダッシュ）演出・ハッシュタグ・絵文字。150〜220字。""",
        "example": "夜中の3時すぎに目が覚めることありませんか。理由もなく、ただ目が開く。\n\n丑三つ時は「薄い」時間らしい。\n\n目が覚めたとき、あなたはひとりでしたか。確かめましたか。",
    },
]
...
# ai-company/training/hooks_topics_standard.md のフック型カタログ（怪談向けに適用）
_HORROR_HOOK_TYPES = """
【フックの型カタログ（ai-company品質基準より・毎回異なる型を狙う）】
- 引き算型: 数の異常を読者に計算させる（例:「歯ブラシが4本のはずが5本」）
- 目撃者ズレ型: 第三者（子供・ペット）だけが異変を知っている（例:「犬が毎晩同じ壁を見る」）
- 日常破壊型: 毎日の儀式が一点だけ狂う（例:「母は必ず3回ノックする。今夜は2回だった」）
- 権威失敗型: 頼る先（管理会社・警察・メーカー）が機能しない・説明できない
- 過去反転型: あの時の出来事の意味が後から変わる
- 時刻・数字固定型: 具体的な時刻/数字が繰り返し出て錨になる
- 当事者宣告型: 「これはあなたの家でも起きている」と読者を当事者にする
"""

def generate_horror_seeds(count: int = 5) -> list[dict]:
    """
    クリシェを避けた怪談ネタの種をcount個生成する。
    Returns: [{"fact": str}, ...]
    """
    prompt = f"""怪談投稿のネタを{count}個。日常の細かい「おかしな事実」だけ書いてください。

禁止クリシェ:
- 亡くなった家族からLINE・電話・メッセージ
- インターホンが鳴って誰もいない
- 鏡・写真に人が映る
- 「今お前の後ろにいる」系

{_HORROR_HOOK_TYPES}
{count}個それぞれ別の型を1つずつ選んで書くこと（同じ型を2回使わない）。

良いネタの条件:
- 「その事実を知ったとき、じわじわくる」程度のもの
- 大げさでない、説明しにくい違和感
- 具体的な数字・場所・行動がある（抽象語「怖い」「不思議な」は使わない・1点物にする）
- 数字を使う場合は計算が合っているか確認する（例:「4年で9人・平均2ヶ月」は矛盾→4年なら約24人が正しい）
- 数字はリアルな粒度にする。「1.8m先」「2時17分」など実際に誰も測らない・覚えていない細かさは使わない。「壁際まで」「2時すぎ」程度の自然な表現にする

JSON形式のみ出力:
{{"seeds": [{{"fact": "事実だけ2〜3文", "hook_type": "使った型の名前"}}]}}"""

    import json as _json
    raw = _call_claude(prompt, max_tokens=800)
    try:
        s = raw.find("{"); e = raw.rfind("}") + 1
        return _json.loads(raw[s:e]).get("seeds", [])
    except Exception:
        return []


_KAIKOWA_BAD_EXAMPLES = """
【これはダメ——AI臭い・不自然な理由付き】
× 「私の影が、一つ多かった。」→ オチを宣言している
× 「では、〜は誰だったのだろう。」→ 疑問文でオチを説明している
× 「〜のかもしれない」「〜のだろう」「〜ではないだろうか」→ AI特有の締め
× 箇条書き（①②③）→ SNS投稿らしくない
× 文と文の間に論理的なつながりがない→ 読んでいて話が追えない
× 突然話題が変わる（「くしゃみが2回→なぜか不動産屋に聞く」みたいな飛躍）

【良い文章の特徴】
- 1文ごとに「なぜ次の行動・発見があったか」が自然につながっている
- 読者が話の流れを追えるよう、「〜したら」「〜だったので」「それで」でつなぐ
- 事実を並べるだけで終わる。読者が勝手に気づく
- 本人も困惑したまま終わる（「今でもわからない」「よく覚えてない」）
- 口語：「〜してた」「〜だったんだけど」「〜したら〜だった」
- 1つの体験を1本の線として語る。途中で別の話題に飛ばない
"""

_KAIKOWA_POST_RULES = """
禁止語尾・表現: 「〜ものだ/ものだった」「〜のだろう」「〜ではないだろうか」「〜たのかもしれない」「〜のだった」
禁止パターン: オチを疑問文で締める・伏線→回収の明示・「〜だけ違った」でオチを宣言・「死んだ」（→「亡くなった」）
ハッシュタグ・絵文字なし。150〜200字。
"""

# 基準例。今後の生成品質はこれを最低ラインとする（2026-06-27更新）。
_KAIKOWA_GOLD_EXAMPLES = {
    "日常どんでん返し": "引っ越してから気づいたんだけど、この部屋に前の住人のカレンダーが残ってた。毎週月曜に「可燃ごみ」って書いてあるだけで、他は何もない。全部のページそうなのに、11月だけ書き込みが一切なかった。おかしいと思って日付を数えてみたら、11月が31日まであった。",
    "実話体験談": "うちの犬が亡くなって三ヶ月後、母が押し入れを片付けてたら散歩用のリードが二本出てきた。一匹しか飼ってなかったし、買った記憶も一本しかないって家族全員が言う。二本とも同じくらい使い込んであって、どちらが元からあったやつかもわからなかった。母はしばらくそれを手に持ったまま、何も言わなかった。",
    "意味がわかると怖い": "近所のコンビニに7年、週4〜5回通ってる。毎回同じレジに並ぶこともあるのに、一度も顔を覚えられたことがない。気になって、ある日わざと「いつもありがとうございます」って言ってみた。店員さんは「はじめまして」って笑顔で返してきた。帰り道、ふと思った。私、ちゃんとレジ通ってたっけ。",
    "数字フック": "祖父の家を片付けてたら、引き出しから封筒が3通出てきた。未開封で、差出人が祖父の名前。宛先も同じ住所。消印もある。郵便局で調べてもらったら、最後の1通だけ、祖父が亡くなった日の消印だった。中身は、まだ開けてない。",
    "問いかけ終わり": "友人と二人で旅行したとき、チェックインでフロントの人に「お三方ですね」と言われた。聞き間違いだと思ってそのまま部屋に入った。二泊して、特に何もなかった。帰ってから友人に確認したら、友人にも同じように聞こえていたと言った。二人とも、はっきり「三人」と聞いていた。あなたは旅先で、知らない誰かと同じ部屋で眠ったことがありますか。",
}

_KAIKOWA_PATTERNS = list(_KAIKOWA_GOLD_EXAMPLES.keys())


def _check_post_with_agents(text: str) -> str:
    """
    fact_checker（論理矛盾）→ buzz_analyst（バズ評価）の2エージェントで投稿をレビューし、
    改善済みテキストを返す。
    """
    from multi_agent import AGENT_PERSONAS

    # Step1: fact_checker — 話の筋が通っているか
    fc_prompt = f"""{AGENT_PERSONAS['fact_checker']}

以下の怪談X投稿の論理的な矛盾・意味の通じない箇所を検出してください。
...
- 「怖い点」が読者に明確に伝わるか
- 原因と結果の関係がわかるか
- 締めが話の内容と論理的につながっているか
- 「自分でも意味わからない」などオチを自分で壊していないか
- 数字の整合性（例:「4年で9人・平均2ヶ月以内に退去」は矛盾）
- 数字の粒度（「1.8m先」「2時17分」など誰も実際に測らない・覚えていない細かさは不自然。「壁際まで」「2時すぎ」など実生活で使う表現にする）

JSON形式のみ出力:
{{"has_issue": false, "fix_instruction": ""}}"""

    fc_data = _safe_json(_call_claude(fc_prompt, max_tokens=300), {"has_issue": False, "fix_instruction": ""})

    if fc_data.get("has_issue") and fc_data.get("fix_instruction"):
        text = _call_claude(f"""以下の怪談X投稿を修正してください。
...
修正指示: {fc_data['fix_instruction']}

禁止: ハッシュタグ・絵文字・「〜ものだ」「〜のだろう」・感情説明・「死んだ」（→「亡くなった」）
150〜200字。投稿文だけ出力。""", max_tokens=400).strip()

    # Step2: buzz_analyst — バズ観点で改善
    ba_prompt = f"""{AGENT_PERSONAS['buzz_analyst']}

以下の怪談X投稿を評価し、より読者を引きつける表現に改善してください。
...
- 冒頭1〜2文で読み続けたくなるか
- 「じわじわ来る」素っ気なさがあるか
- AIっぽい締め（「〜のだろう」「〜ものだ」）になっていないか

改善が必要なら書き直す。問題なければそのまま返す。
投稿文だけ出力。"""

    return _call_claude(ba_prompt, max_tokens=400).strip()


def generate_kaikowa_posts(trend_hint: str = "", use_reddit_seeds: bool = True) -> list[tuple[str, str]]:
    """
    クリシェを避けた怪談投稿を5本生成して返す。
    ネタ元はReddit実話（Grok経由・優先）→ 不足分をClaude生成の種で補う。
    Returns: [(pattern_name, post_text), ...]
    """
    seeds: list[dict] = []
    if use_reddit_seeds:
        try:
            from viral_research import fetch_reddit_horror_seeds, grok_available
            if grok_available():
                seeds = fetch_reddit_horror_seeds(5)
        except Exception:
            seeds = []

    if len(seeds) < 5:
        seeds += generate_horror_seeds(5 - len(seeds))

    if not seeds:
        return []

    trend_block = f"\n今日の日本のトレンド（自然に絡む場合だけ使う・無理なら無視）:\n{trend_hint}" if trend_hint else ""
    results = []
    for seed, pattern in zip(seeds, _KAIKOWA_PATTERNS):
        prompt = f"""怪談Xアカウントの投稿を書く。

ネタ（事実）:
{seed.get('fact', seed.get('situation', ''))}

構文: {pattern}

【基準例 — この品質・トーン・つながりを完全に再現すること（内容は上のネタで書く）】
{_KAIKOWA_GOLD_EXAMPLES[pattern]}

基準例のポイント:
- 1文ごとに前の文を受けて次につながっている（「気のせいだと思って寝たんだけど、次の日も」）
- 話者が自分で確かめる行動を取る（「試しに〜してみた」）→その結果がオチ
- 数字や設定に矛盾がない
- 締めは短く、説明しない

{_KAIKOWA_BAD_EXAMPLES}

{_KAIKOWA_POST_RULES}{trend_block}

投稿文だけ出力。"""
        text = _call_claude(prompt, max_tokens=400).strip()
        text = _check_post_with_agents(text)
        # 禁止語の機械的ガード（エージェントのすり抜け対策）
        text = text.replace("死んだ", "亡くなった").replace("死んでいた", "亡くなっていた")
